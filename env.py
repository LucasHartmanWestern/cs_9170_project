...
class Environment:
    def __init__(
        ...
        # -----------------------------
        # Delta-action controls (NEW)
        # -----------------------------
        use_delta_actions: bool = True,        # interpret action as Δ instead of absolute
        delta_scale: float = 0.10,             # multiply raw action by this before applying
        delta_clip: float | None = 0.20,       # per-dim clamp on Δ (None disables)
        pca_clip: float | None = None,         # per-dim clamp on PCA coords (None disables)

        # Optional: keep points within a reasonable radius band (helps prevent blow-ups)
        use_radius_clip: bool = True,
        radius_clip: float | None = 8.5,      # e.g. set to minority p90 or p95 (None disables)
        eps: float = 1e-8,
    ):
        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # deterministic-ish for env sampling (you already seed torch elsewhere)
        np.random.seed(seed)
        ...
        # Radius clip config
        self.use_radius_clip = bool(use_radius_clip)
        ...
        self.num_active_dims = self.A  # updated per episode in reset()
    ...
    # Curriculum helpers
    # --------------------------------------------------
    def _num_active_from_progress(self, episode_idx: int) -> int:
        ...
        if self.total_episodes <= 1:
            ...
            progress = float(episode_idx) / float(self.total_episodes - 1)

        k = self.A
        for threshold, active_dims in self.curriculum_stages:
            if progress >= threshold:
                k = active_dims
            else:
                break
        return max(1, min(k, self.A))
    ...
    def _apply_radius_clip(self, x: torch.Tensor) -> torch.Tensor:
        """
        If enabled, project x onto ball of radius `radius_clip` if ||x|| is too large.
        """
    ...
    def reset(self, episode_idx: int = 0) -> torch.Tensor:
        ...
        if self.curriculum:
            new_num_active = self._num_active_from_progress(episode_idx)

            if episode_idx == 0:
                self.current_stage = 0
            else:
                if new_num_active > self.num_active_dims:
    ...
    def step(self, action: torch.Tensor, curr_length: int):
        ...
        if self.curriculum:
            ...
            if self.use_delta_actions:
                # Interpret action as Δ
                delta = action

                # scale (so raw policy outputs can be ~N(0,1) but steps are small)
                ...
                # absolute action mode (your old behavior)
                new_pca = self.current_pca.clone()
