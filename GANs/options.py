...
class Options():
    ...
    def parse(self):
        ...
        # set gpu ids
        if self.opt.device == 'gpu' and torch.cuda.is_available():
        ...
        if self.opt.name == 'experiment_name':
            exp_num = 1
            if os.path.isdir(os.path.join(self.opt.outf, self.opt.model)):
                for folder in os.listdir(os.path.join(self.opt.outf, self.opt.model)):
                    num = int(folder.split('_')[-1])
                    if num >= exp_num:
                        exp_num = num + 1
            self.opt.name = os.path.join(self.opt.model, f"experiment_{exp_num}")
