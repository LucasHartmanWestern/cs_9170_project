...
METRICS = [
    ...
    ("dp_diff",      "DP' Score",  True),
    ("eod_max_diff", "EOd' Score", True),
]
...
METHOD_STYLES = {
    ...
    "GroupDRO":     {"color": "tab:green",  "ls": "--", "label": "GDRO"},
    "FairTabDDPM":  {"color": "tab:orange", "ls": "--", "label": "FairTabDDPM"},
}
...
for ax, dataset, panel_label in zip(axes, DATASETS, PANEL_LABELS):
    ...
    for tick, lbl in zip(ax.xaxis.get_major_ticks(), ax.get_xticklabels()):
        ...
        if text == "Accuracy":
            ...
            lbl.set_ha("right")
        if text in ("ROC-AUC", "F1 Weighted"):
            tick.set_pad(14)
        else:
            tick.set_pad(6)
    ax.set_ylim(0, 1)
