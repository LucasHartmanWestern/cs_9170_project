# Reinforcement Learning for Fairness-Aware Synthetic Data Generation under Positive-Class Scarcity

Machine learning systems are increasingly used in high-stakes domains such as hiring, healthcare, and lending, where unfair decisions can produce measurable societal harm at scale. Prior evidence shows that, in hiring, men were 6.8% more likely to be selected than women, White candidates 18% more likely than Black candidates, and graduates from elite universities 21.3% more likely than those from lower-ranked institutions. In finance, reviews of 78 studies and analyses of 157,269 loan applications have reported persistent algorithmic discrimination. These findings highlight the need for fairness-aware learning, particularly where disadvantaged groups have very few positive-class examples. In such settings, no conventional baseline achieves consistent fairness improvement across both datasets while maintaining competitive utility: reweighting and loss-shaping methods show inconsistent gains under severe positive-class scarcity, and untargeted generative methods yield diminishing returns when the disadvantaged group's positive-class supply is too sparse to learn from. To address this, fairness optimization via reinforcement-guided by equity (FORGE) is proposed: a reinforcement learning agent generates synthetic minority-positive training samples guided by worst-group validation loss, operating in a PCA-compressed feature space with bounded delta actions, without modifying the downstream classifier objective. Experiments on real-world datasets show that no baseline matches FORGE's fairness and utility profile, whereas FORGE reduces the equal opportunity gap by 53% to 92%.
