...
class DecisionTransformer(TrajectoryModel):
    ...
    def get_predictions(
        ...
        eval_context_length = self.eval_context_length if external_context_len is None else external_context_len
        ...
        if self.max_length is not None:
            states = states[:, -eval_context_length :]
            actions = actions[:, -eval_context_length :]
            returns_to_go = returns_to_go[:, -eval_context_length :]
            timesteps = timesteps[:, -eval_context_length :]
        ...
        if self.stochastic_policy:
            ...
            return (
                state_preds[:, -1],
                self.clamp_action(action_preds),
                return_preds[:, -1],
            )
