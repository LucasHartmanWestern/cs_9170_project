...
DIRS = {
    "census": [
        FORGE / "training_runs",
        FORGE / "lambda_runs/census/k3",
        FORGE / "aulavik_runs/census/k5",
        FORGE / "training_runs_k10",
    ],
    "capture24": [
        FORGE / "aulavik_runs/capture_24/k0",
        FORGE / "lambda_runs/capture_24/k3",
        FORGE / "aulavik_runs/capture_24/k5",
        FORGE / "lambda_runs/capture_24/k5",
    ],
}

DATASET_FILTER = {
    "census":    lambda n: n.startswith("SPECcensus"),
    "capture24": lambda n: n.startswith("SPECcapture24"),
}

MIN_SEEDS = 2  # set to 1 once all runs are complete

# Best baseline beta-EO per dataset (SMOTE census=FLB, capture24=SMOTE)
BEST_BASELINE = {"census": 0.039, "capture24": 0.057}
BASELINE_LABEL = {"census": "Best baseline (FLB)", "capture24": "Best baseline (SMOTE)"}
...
def read_run(run_dir):
    ...
    if not (run_dir / "final_test_metrics.csv").exists():
        return None
    meta = {}
    ...
    traj = parse_traj(run_dir.name)
    if k is None:
        m = re.search(r"_k(\d+)_", run_dir.name); k = int(m.group(1)) if m else None
    if pca is None:
        m = re.search(r"_PCA(\d+)_", run_dir.name); pca = int(m.group(1)) if m else None
    if ep is None:
        m = re.search(r"_e(\d+)(?:_|$)", run_dir.name); ep = int(m.group(1)) if m else 10
    if any(v is None for v in [k, pca, traj, ep]):
        return None
    try:
        df = pd.read_csv(run_dir / "final_test_metrics.csv")
        eo  = df["beta_eo_tpr_diff"].dropna().values
        auc = df["beta_roc_auc"].dropna().values if "beta_roc_auc" in df.columns else np.array([])
        if len(eo) == 0:
            return None
        return dict(
            k=float(k), pca=int(pca), ep=int(ep),
...
def load_dataset(dataset):
    records = []
    filt = DATASET_FILTER[dataset]
    for root in DIRS[dataset]:
        root = Path(root)
        if not root.exists():
            continue
        for d in sorted(root.iterdir()):
            if d.is_dir() and filt(d.name):
                r = read_run(d)
                if r:
                    records.append(r)
