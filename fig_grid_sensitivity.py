...
def parse_traj_from_dirname(name):
    m = re.search(r"_TRJ(\d+)_", name)
    return int(m.group(1)) if m else None


def read_run(run_dir):
    """Return hyperparams + mean/std beta-EO for one run directory, or None."""
    ...
    try:
        ...
        eo_vals = df["beta_eo_tpr_diff"].dropna().values
        if len(eo_vals) == 0:
            return None
        return dict(
            k=float(k), pca=int(pca), traj=int(traj), ep=int(ep),
            beta_eo_mean=float(np.mean(eo_vals)),
            beta_eo_std=float(np.std(eo_vals)),
            n_seeds=len(eo_vals),
        )
...
def load_dataset(dataset):
    ...
    for root in DIRS[dataset]:
        ...
        for run_dir in sorted(root.iterdir()):
            if run_dir.is_dir() and filt(run_dir.name):
