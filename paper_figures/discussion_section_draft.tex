...
The credit card results illustrate the complementary boundary condition.
Because the no-intervention alpha model already achieves near-zero EO on that
dataset, the disadvantaged group's positive rate is not low enough to create
meaningful bias in the learned classifier.
All methods, including ours, produce modest and statistically indistinguishable
EO changes.
This is expected behaviour: when there is no fairness gap to close, a
generative augmentation framework has no target to optimise toward.
The credit results therefore do not undermine the main claim — they define
the regime boundary beyond which the framework's advantage diminishes.
